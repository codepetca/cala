  ...
  return (
    <div
      ...
      className={`
        ...
        ${isDragging ? 'opacity-30 scale-95 rotate-2' : 'hover:shadow-md hover:scale-105'}
        ${className}
      `}
    >
        ...
        {events.slice(0, 3).map((event, index) => (
          <div key={event._id} className="relative">
            <DraggableEvent
              ...
              className={isDragOver && insertIndex === index + 1 ? 'transform translate-y-1 transition-transform duration-200' : ''}
            />
  ...
  return (
    ...
    >
      <div className="h-screen flex flex-col bg-gray-50">
        ...
        <DragOverlay>
          {activeEvent && (
            <div className="transform rotate-6 scale-110 drop-shadow-2xl pointer-events-none">
              <DraggableEvent
                event={activeEvent}
                onEdit={() => {}}
                onDelete={() => {}}
                onClick={() => {}}
                isSelected={false}
                className="opacity-90 shadow-2xl border-2 border-blue-400 bg-opacity-95"
              />
            </div>
          )}
        </DragOverlay>
